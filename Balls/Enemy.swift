...
class Enemy: GameSprite {
    ...
    override class func add(scene:SKScene) {
        ...
        let actualY = mmrandom(min: scene.size.height * 0.15, scene.size.height * 0.85 )
        ...
        let actualDuration = random(min: CGFloat(10.0), CGFloat(16.0))
        
        // Determine size
        let actualScale = (random(min: CGFloat(1.0), CGFloat(6.0)))
        monster.setScale(actualScale / 6.0)
        
        // Create the actions
        
        let ydelta = (random(min: CGFloat(-500.0), CGFloat(500.0)))
        let actionMove = SKAction.moveTo(CGPoint(x: -scene.size.width/2, y: actualY + ydelta), duration: NSTimeInterval(actualDuration))
        
        let actionRotate = SKAction.rotateByAngle(random(min: CGFloat(-1.0), CGFloat(1.0))*CGFloat(M_PI), duration:1)
